...
import { Users, ArrowRight, AlertCircle } from "lucide-react"; // AlertCircle no longer used but keeping for completeness.
...
export default function StanceSelector({ debate, participantCounts, userName, onTakeStance, isSubmitting }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (selectedPosition) {
      // userName is now passed as a prop, assuming it's already validated and available.
      // The onTakeStance callback will now receive only the selected position,
      // as the userName is managed externally.
